...
class MWBCommunicator: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate
{
    ...
    private var commandQueue: MWQueue<MWBCommand> = MWQueue()
    ...
    ///
    /// If you want to use the app's Bluetooth communicator, use `myWatch.get().bluetoothCommunicator`
    override init()
    {
        super.init()
    }
    ...
    {
        //Log error message
        MWLError("Was unable to connect to peripheral: \"\(peripheral.name!)\", more details: \(error?.localizedDescription ?? "<no more details>")", module: .moduleBluetooth)
    }
    ...
    internal func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?)
    {
        ...
        {
            //If not, log an error message
            MWLError("Was unable to discover characteristics on the connected myWatch device, because the service is not the Uart service, more details: \(error?.localizedDescription ?? "<no more details>")", module: .moduleBluetooth)
        }
    ...
    internal func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?)
    {
        ...
        if(characteristic == uartTxCharacteristic && commandQueue.count >= 1)
        {
            ...
            //Log information message
            MWLInfo("Successfully delivered command: \(commandQueue.peek().command) to the connected myWatch device.", module: .moduleBluetooth)
    ...
    func initializeBluetooth(with delegate: MWBCommunicatorDelegate)
    {
    ...
    func initializeBluetooth(with delegate: MWBCommunicatorDelegate, device: MWDevice)
    {
        self.delegate = delegate
        self.device = device
        self.centralManager = CBCentralManager(delegate: self, queue: nil, options: nil)
    ...
    func sendCommand(_ command: MWBCommand)
    {
