...
@objc protocol MWBCommunicatorDelegate
{
    ...
    ///Called when the CBCentralManager in MWBCommunicator updates its state to '.poweredOn'.
    @objc optional func bluetoothHasBeenEnabled()
    
    ///Called when function 'centralManagerDidUpdateState' function gave a state which is not '.poweredOn'.
    @objc optional func bluetoothNotAvailable()
    
    ///Called when the CBCentralManager in MWBCommunicator found and constructed a myWatch device.
    @objc optional func bluetoothHasFoundDevice(_ device: MWDevice)
    
    ///Called when the MWBCommunicator has found the specified device in the initializer.
    @objc optional func bluetoothHasFoundSpecifiedDevice()
    
    ///Called when the MWBCommunicator successfully connected to a myWatch device.
    @objc optional func connectionSuccessful(to device: MWDevice)
    
    ///Called when both the Tx and Rx characteristics were discovered by MWBCommunicator.
    @objc optional func deviceIsReadyToUse(_ device: MWDevice)
    
    ///Called when the Rx characteristic's value changes as a response to the previously sent command.
